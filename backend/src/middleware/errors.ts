...
export function onError(err: Error, c: Context<AppEnv>): Response {
  ...
  if (err instanceof ApiError) {
    // Clients back off far more precisely with a hint than with a guess. The
    // shared limiter reports no reset time, so this is a fixed, conservative
    // floor rather than an exact one.
    const headers = err.status === 429 ? { 'retry-after': '2' } : undefined;
    return c.json(
