    ...
    (
      await env.DB.prepare(
        `SELECT id, username, display_name, avatar_key, avatar_original_key,
                avatar_square_key, accent, flags FROM users WHERE id IN (${ph(authorIds.length)})`,
      )
        .bind(...authorIds)
        .all<{
      ...
      // is_bot rides on every author so clients can tag bot replies without a
      // second lookup; flags itself stays server-side.
    ).results.map(({ flags, ...u }) => [u.id, { ...u, is_bot: isBotUser(flags) }]),
  );
